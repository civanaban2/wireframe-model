...
/*   Updated: 2025/03/27 05:05:10 by cari             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotation_hook(t_core *core, int keycode)
{
	if (keycode == XK_w || keycode == XK_W)
		...
		core->camera.z_rotation -= M_PI / 180 * 5;
...
void	translation_hook(t_core *core, int keycode)
{
	if (keycode == XK_Up)
		...
		core->camera.x_trans += 10;
	if (core->camera.x_trans < -10000)
		core->camera.x_trans = -10000;
	else if (core->camera.x_trans > 10000)
		core->camera.x_trans = 10000;
	if (core->camera.y_trans < -10000)
		core->camera.y_rotation = -10000;
	else if (core->camera.y_trans > 10000)
		core->camera.y_trans = 10000;
}
...
void	color_hook(t_core *core, int keycode)
{
	if (keycode == XK_1)
		...
		core->camera.color_mode = 5;
	else if (keycode == XK_6)
		core->camera.color_mode = 6;
}
